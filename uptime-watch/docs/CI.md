...
Use the contents of `uptime-watch` as the root of its own GitLab repository. Its `.gitlab-ci.yml` runs Python tests, checks Bash/Ansible syntax, validates Terraform, executes the Docker smoke test, and publishes an image tagged with the commit SHA. Deployment is a separate manual job on the default branch.
...
The deployment job expects a protected **Linux shell runner** tagged `uptime-watch-deploy`, with Bash, kubectl and connectivity to the cluster. It has no container image because it deliberately uses that runner's installed tools. The job uses a resource group to serialize deployments.
...
kubectl --context "$KUBE_CONTEXT" -n uptime-watch create secret docker-registry registry-pull \
  --docker-server="$REGISTRY" --docker-username="$REGISTRY_USER" --docker-password="$REGISTRY_TOKEN"
kubectl --context "$KUBE_CONTEXT" -n uptime-watch patch serviceaccount default \
...
Review the smoke test result, then run the manual deployment job. Deployment does not execute Terraform apply or destroy. Follow the README's post-deployment verification before considering a release complete.
