...
This optional route provisions a dedicated Ubuntu EC2 host for `uptime-watch`. Local Docker and kind runs do not need AWS. Terraform creates a VPC, a public subnet, an SSH-only security group, a t3.medium instance and an encrypted 30 GiB disk. Compute, storage and public IPv4 usage incur charges.
