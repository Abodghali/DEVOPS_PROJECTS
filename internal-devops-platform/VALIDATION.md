...
- Strict Kubernetes 1.34 schema validation for 9 core resources, including rendered placeholders where applicable.
- 2 custom resource manifests checked against the schemas from the pinned monitoring/database charts. This does not evaluate admission CEL or reconcile controllers.
...
The HTTP integration test polls the local GitLab fixture and checks its pipeline duration, outcome and runner status. It does not access a real GitLab account.
